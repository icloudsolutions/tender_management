...
class ShPurchaseAgreement(models.Model):
    ...
    def action_validate(self):
        ...
        if self:
            for rec in self:
                if self.env.company.sh_auto_add_followers and rec.sh_partner_ids:
                    rec.message_subscribe(rec.sh_partner_ids.ids)
                if self.env.company.sh_portal_user_create and rec.sh_partner_ids:
                    for vendor in rec.sh_partner_ids:
    ...
    def action_send_tender(self):
        ...
        try:
            template_id = ir_model_data._xmlid_lookup('sh_all_in_one_tender_bundle.email_template_edi_purchase_tedner')[1]
        except ValueError:
            template_id = False
        try:
        ...
        if self.sh_partner_ids:
            ctx.update({
                'default_partner_ids': [(6, 0, self.sh_partner_ids.ids)]
            })
    ...
    def action_view_order(self):
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'tree,form',
            'domain': [('agreement_id', '=', self.id), ('state', 'in', ['purchase','done'])],
            'target': 'current'
        }
    ...
    def sh_action_create_multiple_rfq(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'context':{'default_sh_partner_ids':[(6,0,self.sh_partner_ids.ids or [])],'default_sh_mail_template_id':self.env.ref('purchase.email_template_edi_purchase').id or False},
            'target': 'new'
        }
