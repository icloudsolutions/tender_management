    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if request.env.user.is_tendor_vendor:
            if request.env.user.has_group('sh_all_in_one_tender_bundle.sh_purchase_tender_user'):
                ...
                tender_count = tender_obj.sudo().search_count([('sh_open_tender','=',False),('state', 'not in', ['draft']), (
                    'sh_partner_ids', 'in', [request.env.user.partner_id.id])])
                values['tender_count'] = tender_count
        return values
    ...
    def portal_my_home_tender(self, page=1):
        ...
        if not request.env.user.has_group('sh_all_in_one_tender_bundle.sh_purchase_tender_user') or not request.env.user.has_group('sh_all_in_one_tender_bundle.sh_purchase_tender_manager'):
            searchbar_filters.update({
                'all': {'label': _('All'), 'domain': [('sh_open_tender', '=', False), ('state', 'in', ['confirm', 'bid_selection']), ('sh_partner_ids', 'in', [request.env.user.partner_id.id])]},
                'draft': {'label': _('Closed'), 'domain': [('sh_open_tender', '=', False), ('state', '=', 'closed'), ('sh_partner_ids', 'in', [request.env.user.partner_id.id])]},
                'cancel': {'label': _('Cancelled'), 'domain': [('sh_open_tender', '=', False), ('state', '=', ['cancel']), ('sh_partner_ids', 'in', [request.env.user.partner_id.id])]},
            })
        else:
            searchbar_filters.update({
        ...
        try:
            if not request.env.user.has_group('sh_all_in_one_tender_bundle.sh_purchase_tender_user'):
                if tender.sh_open_tender:
                    ...
                    if tender.sh_allow_portal_access:
                        if tender.sh_partner_ids and request.env.user.partner_id.id in tender.sh_partner_ids.ids:
                            tender_sudo = self._document_check_access(
                                'purchase.agreement', tender_id, access_token=access_token)
                        elif tender.sh_partner_ids and request.env.user.partner_id.id not in tender.sh_partner_ids.ids:
                            if access_token:
                                tender_sudo = self._document_check_access(
                                    'purchase.agreement', tender_id, access_token=access_token)
                            else:
                        ...
                        if tender.sh_partner_ids:
                            if request.env.user.partner_id.id in tender.sh_partner_ids.ids:
                                tender_sudo = self._document_check_access(
                                    'purchase.agreement', tender_id, access_token=None)
                            elif request.env.user.partner_id.id not in tender.sh_partner_ids.ids:
                                return request.redirect('/my')
                        else:
                            return request.redirect('/my')
